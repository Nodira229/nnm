...
Select * from Employees
 ...
 declare @full_name varchar(100) = '100 Steven King'

 declare @first_space int = (select CHARINDEX(' ',@full_name,1))
 select @first_space
 declare @second_space int = (select CHARINDEX(' ',@full_name,@first_space + 1))
 select @second_space
 declare @legth_word int = (select len(@full_name))

 select 
  left(@full_name,@first_space - 1) as EMPLOYEE_ID,
  SUBSTRING(@full_name,@first_space + 1,@second_space - @first_space) as firstname,
  right(@full_name,@legth_word - @second_space) as LAST_NAME


  SELECT REPLACE('650.124.8234', '124','999')
...
declare @FIRST_NAME varchar(100)
  ...
  select DATALENGTH(@FIRST_NAME) AS NAME_LENGTH
	FROM Employees
WHERE 
    first_name LIKE 'A%' 
...
VALUES
	...
	DECLARE @YEAR VARCHAR (30) = '2001'

	SELECT @YEAR,MAX(101) FROM TestMax
